...
export default function SectionSkills({ onError }: onErrorProps) {
     const { loading, error, data } = useQuery<SkillsData>(GET_SKILLS_DATA);
     ...
     return (
          ...
          >
               <div className="w-10/12 sm:w-8/12">
                    <div className="tabtitle">{data?.skill.section}</div>
                    <h2>{data?.skill.title}</h2>
                    <p className="mt-5 mb-5">{data?.skill.description}</p>
               </div>
               <animated.div
                    ref={refSkills}
               ...
               >
                    ...
                    >
                         {data?.skill.skillCards
                              .slice(
                                   data.skill.skillCards.length / 2,
                                   data.skill.skillCards.length
                              )
                              .map((skill) => (
                                   <SkillCard
                                        key={skill.id}
                    ...
                    >
                         {data?.skill.skillCards
                              .slice(0, data.skill.skillCards.length / 2)
                              .map((skill) => (
                                   <SkillCard
                                        key={skill.id}
                                        label={skill.label}
